...
object DesResponse {
  ...
  val jsonResponse422: JsValue = Json.parse(s"""
       |{
  ...
  val jsonResponseInvalidIdType: JsValue = Json.parse(s"""
       |{
       | "code": "INVALID_IDTYPE",
       | "reason": "Submission has not passed validation. Invalid parameter idType."
       |}""".stripMargin)

  val jsonResponseInvalidIdValue: JsValue = Json.parse(s"""
       |{
       | "code": "INVALID_ID",
       | "reason": "Submission has not passed validation. Invalid parameter idValue."
       |}""".stripMargin)

  val jsonResponseInvalidCorrelationId: JsValue = Json.parse(s"""
       |{
